  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-8">
        <div>
          <h1 className="text-5xl md:text-7xl font-syne font-black tracking-tighter uppercase mb-4">THE SHOP</h1>
          <p className="text-zinc-500 max-w-md font-medium">Limited release jerseys from our signature series. Ready to wear, ready to flex.</p>
        </div>
        ...
        {filtered.map((product) => (
          <div key={product.id} className="group relative">
            <div className="aspect-[3/4] overflow-hidden rounded-2xl bg-zinc-100 dark:bg-zinc-900 relative">
              ...
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center space-x-4">
                <button 
                  ...
                  className="p-4 bg-white text-black rounded-full hover:bg-amber-500 transition-colors shadow-xl"
                >
            ...
            <div className="mt-6 flex justify-between items-start">
              <div>
                <h3 className="font-syne font-bold text-xl uppercase tracking-tighter">{product.name}</h3>
                <p className="text-zinc-500 text-sm mt-1">{product.description.substring(0, 50)}...</p>
              </div>
              <span className="font-syne font-black text-2xl text-amber-500">${product.price}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-20 opacity-50">
          <p className="text-2xl font-bold">NO JERSEYS FOUND IN THIS CATEGORY</p>
        </div>
      )}
    </div>
  );
